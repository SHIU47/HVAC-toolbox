setInterval(() => {
        ...
        const DTC = {
            ...
            init() {
                ...
                this.renderer.toneMappingExposure = 1.5; // 整體場景明亮度提升（曝光係數，全域生效）
            ...
            applyCoupledData(data) {
                ...
                // 將計算請求發送至背景 PUE 計算服務（server.js on port 3000）
                // 核心計算公式已移至伺服器端，前端不再保存演算法細節
                const payload = {
                    whitespaceLoad: data.whitespaceLoad || 1200,
                    chillerPower: data.chillerPower || null,
                    mepScenario: data.mepScenario || ''
                };
                
                fetch('http://127.0.0.1:3000/api/calculate-pue', {
                    method: 'POST',
                ...
                .then(result => {
                    if (result.success) {
                        ...
                        if (pueEl) pueEl.setAttribute('data-base', result.pue.toFixed(3));
                    }
                })
                .catch(err => {
                    console.warn('[applyCoupledData] PUE 計算服務暫時無法連線，使用預設值。', err.message);
                });
            },
            ...
            showDetail(data) {
                ...
                document.getElementById('detail-content-fanwall').style.display = data.type === 'Fanwall' ? 'block' : 'none';
                ...
                if(data.type === 'Fanwall') document.getElementById('detail-subtitle').innerText = 'CRAH FANWALL ‧ 冰水盤管 + EC 節能風機';
