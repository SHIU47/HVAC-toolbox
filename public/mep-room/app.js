const APP = {
            ...
            createMaterials() {
                ...
                // 新增高精度發電機 HMI 動態螢幕貼圖
                const genC = document.createElement('canvas'); genC.width = 512; genC.height = 256;
                const genCtx = genC.getContext('2d');
                genCtx.fillStyle = '#020617'; genCtx.fillRect(0, 0, 512, 256);
                genCtx.fillStyle = '#1e293b'; genCtx.fillRect(0, 0, 512, 42);
                genCtx.fillStyle = '#38bdf8'; genCtx.font = 'bold 18px monospace'; genCtx.fillText('● DIESEL GENERATOR HMI v2.0', 14, 28);
                this.textures.generatorScreen = new THREE.CanvasTexture(genC);
            },
            ...
            addCabinetDoor(parent, width, height, frontZ, type = 'glass') {
                const doorGroup = new THREE.Group();
                
                // 門鉸鏈是在左側，即 x = -width / 2
                const hingeX = -width / 2;
                doorGroup.position.set(hingeX, 1.14, frontZ);
                
                // 門板在 doorGroup 的 local space 中
                // 為了讓鉸鏈在 (0, 0, 0) 旋轉，門板的中心位置在 local x = width / 2
                const localX = width / 2;
                
                // 1. 強化版黑金屬門框 (更厚實，增加立體感)
                ...
                // 門的四個邊框
                [
                    { size: [frameThickness, height, frameDepth], pos: [0, 0, 0] }, // 左側鉸鏈邊框
                    { size: [frameThickness, height, frameDepth], pos: [width, 0, 0] }, // 右側把手邊框
                    { size: [width, frameThickness, frameDepth], pos: [localX, height / 2, 0] }, // 上邊框
                    { size: [width, frameThickness, frameDepth], pos: [localX, -height / 2, 0] }  // 下邊框
                ].forEach(part => {
                ...
                // 2. 門底部的金屬防撞護板 (Kick-plate) — 增加門板的結構感，讓它看起來極像真正的伺服器門
                const kickHeight = height * 0.18;
                const kickPlate = new THREE.Mesh(
                    new THREE.BoxGeometry(width - 0.04, kickHeight, 0.014),
                    frameMat
                );
                kickPlate.position.set(localX, -height / 2 + kickHeight / 2 + 0.02, 0);
                doorGroup.add(kickPlate);
                
                // 3. 高可視度天藍色自發光玻璃面板 (Glass panel)
                let glassMat;
                if (type === 'glass') {
                    ...
                    // 金屬門板
                    glassMat = this.materials.cabinet;
                }
                
                const glassHeight = height - kickHeight - 0.06;
                const glassY = kickHeight / 2 + 0.01;
                const glass = new THREE.Mesh(
                    new THREE.BoxGeometry(width - 0.04, glassHeight, 0.008),
                    glassMat
                );
                glass.position.set(localX, glassY, 0);
                doorGroup.add(glass);
                
                // 4. 精緻鋼製長型拉手把 (鋼管把手 + 雙端支架)
                const handle = new THREE.Mesh(new THREE.BoxGeometry(0.025, 0.5, 0.025), this.materials.aluminum);
                handle.position.set(width - 0.03, 0, 0.028);
                doorGroup.add(handle);
                // 上支架
                const bracket1 = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.02, 0.03), this.materials.aluminum);
                bracket1.position.set(width - 0.03, 0.22, 0.014);
                // 下支架
                const bracket2 = bracket1.clone();
                bracket2.position.set(width - 0.03, -0.22, 0.014);
                doorGroup.add(bracket1, bracket2);
                
                // 5. 高細節金屬圓柱鉸鏈 (Cylindrical hinges)
                [-height / 3.2, 0, height / 3.2].forEach(y => {
                    const hinge = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, 0.08, 12), this.materials.aluminum);
                    hinge.position.set(0, y, 0.01);
                    doorGroup.add(hinge);
                });
                
                parent.add(doorGroup);
                parent.doorGroup = doorGroup;
                return doorGroup;
            },
            ...
            buildGreySpace() {
                ...
                const createUPSGroup = (xOffset, labelPrefix) => {
                    for(let i=0; i<2; i++) {
                        ...
                        this.addCabinetDoor(ups, 0.74, 1.98, 0.56, 'glass');
                        ...
                        for(let b=0; b<2; b++) {
                            ...
                            this.addCabinetDoor(batt, 0.54, 1.98, 0.575, 'glass');
                ...
                const createPDU = (xPos, label) => {
                    ...
                    this.addCabinetDoor(pdu, 1.12, 1.98, 0.51, 'glass');
                    ...
                    if(isTie) {
                        ...
                        pdu.add(coupler);
                        const couplerHandle = new THREE.Mesh(new THREE.BoxGeometry(0.065, 0.16, 0.032), this.materials.aluminum);
                        couplerHandle.rotation.x = 0.25;
                        couplerHandle.position.set(0.22, 0.72, 0.435);
                        ...
                        [this.materials.yellowLed, this.materials.greenLed].forEach((mat, idx) => {
                            ...
                            l.position.set(-0.22 + idx * 0.44, 0.72, 0.455);
                            pdu.add(l);
                        });
                        this.addLabelPlane(pdu, ['TIE', 'INTERLOCKED'], 0.44, 0.13, { x: -0.06, y: 0.72, z: 0.46 },
                        ...
                        pdu.add(acb);
                        const acbHandle = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.15, 0.03), this.materials.aluminum);
                        acbHandle.rotation.x = 0.25;
                        acbHandle.position.set(0.18, 0.34, 0.435);
                        pdu.add(acbHandle);
                        // 弧室蓋（5條）
                        ...
                        pdu.add(relayScreen);
                        this.addLabelPlane(pdu, ['MAIN ACB', '4000A LSIG'], 0.36, 0.12, { x: 0.12, y: 0.34, z: 0.455 },
                ...
                function createDetailedGenerator() {
                    ...
                    const screenMat = new THREE.MeshBasicMaterial({ map: APP.textures.generatorScreen });
                    const louverBladeMat = new THREE.MeshStandardMaterial({ color: 0x94a3b8, metalness: 0.9, roughness: 0.25 }); // 亮銀灰色金屬，反光對比強烈
                    const grilleBackMat = new THREE.MeshBasicMaterial({ color: 0x050505 }); // 極深黑色背景，加強鏤空感
                    ...
                    // 1. 柴油引擎缸體 (Diesel Engine Block) — 深灰色鑄鐵質感
                    const engineBlockMat = new THREE.MeshStandardMaterial({ color: 0x27272a, metalness: 0.85, roughness: 0.35 });
                    const engineBlock = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.95, 0.85), engineBlockMat);
                    engineBlock.position.set(-0.35, 0.88, 0);
                    engineGroup.add(engineBlock);

                    // 2. 引擎上部汽缸蓋與紅色氣門室蓋 (Valve Cover)
                    const valveCoverMat = new THREE.MeshStandardMaterial({ color: 0xb91c1c, metalness: 0.7, roughness: 0.3 }); // 經典紅色氣門室蓋
                    const valveCover = new THREE.Mesh(new THREE.BoxGeometry(1.9, 0.18, 0.65), valveCoverMat);
                    valveCover.position.set(-0.35, 1.45, 0);
                    engineGroup.add(valveCover);

                    // ── [NEW DETAIL] 氣門室蓋鍍鉻高精度固定螺栓 (Chrome Valve Cover Hex Bolts) ──
                    for (let bx = -0.8; bx <= 0.8; bx += 0.4) {
                        [-0.22, 0.22].forEach(bz => {
                            const bolt = new THREE.Mesh(new THREE.CylinderGeometry(0.014, 0.014, 0.02, 6), APP.materials.aluminum);
                            bolt.position.set(-0.35 + bx, 1.54, bz);
                            engineGroup.add(bolt);
                        });
                    }

                    // ── [NEW DETAIL] 黃色機油加注蓋 (Engine Oil Filler Cap) ──
                    const oilCap = new THREE.Mesh(new THREE.CylinderGeometry(0.042, 0.042, 0.022, 10), canopyMat);
                    oilCap.position.set(-1.1, 1.54, 0.1);
                    engineGroup.add(oilCap);

                    // 3. 雙側排氣岐管與增壓管路 (Exhaust Manifold & Turbo Piping)
                    const manifoldMat = new THREE.MeshStandardMaterial({ color: 0x4b5563, metalness: 0.9, roughness: 0.4 });
                    [-0.26, 0.26].forEach(zOff => {
                        const manifold = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.12, 0.12), manifoldMat);
                        manifold.position.set(-0.35, 1.25, zOff);
                        engineGroup.add(manifold);
                        // 接管
                        for(let p=0; p<4; p++) {
                            const pipe = new THREE.Mesh(new THREE.CylinderGeometry(0.024, 0.024, 0.18, 8), manifoldMat);
                            pipe.rotation.z = Math.PI / 2;
                            pipe.position.set(-0.95 + p * 0.4, 1.15, zOff);
                            engineGroup.add(pipe);
                        }
                    });

                    // 4. 渦輪增壓器本體 (Turbocharger)
                    const turboMat = new THREE.MeshStandardMaterial({ color: 0x71717a, metalness: 0.95, roughness: 0.15 });
                    const turbo = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.28, 16), turboMat);
                    turbo.rotation.z = Math.PI / 2;
                    turbo.position.set(-0.4, 1.62, 0);
                    engineGroup.add(turbo);

                    // ── [NEW DETAIL] 渦輪增壓器排氣管出口 (Turbocharger Exhaust Outlet Line) ──
                    const turboExhaust = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.35, 16), pipeMat);
                    turboExhaust.position.set(-0.4, 1.78, 0);
                    engineGroup.add(turboExhaust);

                    // 5. 發電機端 (Alternator) — 工業銀灰烤漆
                    const alternatorMat = new THREE.MeshStandardMaterial({ color: 0x3f3f46, metalness: 0.75, roughness: 0.25 });
                    const alternator = new THREE.Mesh(new THREE.CylinderGeometry(0.46, 0.46, 1.15, 24), alternatorMat);
                    alternator.rotation.z = Math.PI / 2;
                    alternator.position.set(1.15, 0.88, 0);
                    engineGroup.add(alternator);

                    // 6. 發電機內部的銅繞組 (Copper Stator Windings) — 可從保護罩縫隙中窺見
                    ...
                    const copperStator = new THREE.Mesh(new THREE.CylinderGeometry(0.42, 0.42, 0.45, 16), windingMat);
                    copperStator.rotation.z = Math.PI / 2;
                    copperStator.position.set(1.15, 0.88, 0);
                    engineGroup.add(copperStator);

                    // ── [NEW DETAIL] 發電機內部冷卻風扇 (Alternator Internal Cooling Fan Rotor) ──
                    const altFanGroup = new THREE.Group();
                    altFanGroup.position.set(0.82, 0.88, 0);
                    for (let f = 0; f < 10; f++) {
                        const blade = new THREE.Mesh(new THREE.BoxGeometry(0.008, 0.38, 0.06), frameMat);
                        blade.rotation.x = (f * Math.PI) / 5;
                        blade.rotation.y = 0.2;
                        altFanGroup.add(blade);
                    }
                    engineGroup.add(altFanGroup);

                    // 7. 發電機進出風網柵 (Alternator Vents Cage)
                    const ventCageMat = new THREE.MeshStandardMaterial({ color: 0x18181b, roughness: 0.6, wireframe: true });
                    const ventCage = new THREE.Mesh(new THREE.CylinderGeometry(0.47, 0.47, 1.17, 20, 1, true), ventCageMat);
                    ventCage.rotation.z = Math.PI / 2;
                    ventCage.position.set(1.15, 0.88, 0);
                    engineGroup.add(ventCage);

                    // 8. 引擎散熱水箱風扇 (Cooling Fan Shroud & Blades)
                    const shroudMat = new THREE.MeshStandardMaterial({ color: 0x18181b, metalness: 0.8, roughness: 0.4 });
                    const fanShroud = new THREE.Mesh(new THREE.CylinderGeometry(0.48, 0.48, 0.12, 24, 1, true), shroudMat);
                    fanShroud.rotation.z = Math.PI / 2;
                    fanShroud.position.set(-1.58, 0.88, 0);
                    engineGroup.add(fanShroud);

                    // 風扇扇葉 (Fan Blades)
                    const fanGroup = new THREE.Group();
                    fanGroup.position.set(-1.58, 0.88, 0);
                    for(let f=0; f<8; f++) {
                        const blade = new THREE.Mesh(new THREE.BoxGeometry(0.012, 0.44, 0.14), shroudMat);
                        blade.rotation.x = (f * Math.PI) / 4;
                        blade.rotation.y = 0.26; // 扭轉角
                        fanGroup.add(blade);
                    }
                    engineGroup.add(fanGroup);

                    // ── [NEW DETAIL] 水箱散熱上下水系統橡膠防震軟管 (Cooling Radiator Hoses & Metal Clamps) ──
                    const hoseMat = new THREE.MeshStandardMaterial({ color: 0x0a0a0a, roughness: 0.85 });
                    const clampMat = APP.materials.aluminum;
                    
                    // 上水管
                    const upperHose = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.26, 12), hoseMat);
                    upperHose.rotation.z = Math.PI / 2;
                    upperHose.position.set(-1.42, 1.25, 0.15);
                    engineGroup.add(upperHose);
                    [-1.52, -1.32].forEach(hx => {
                        const clamp = new THREE.Mesh(new THREE.CylinderGeometry(0.044, 0.044, 0.02, 10), clampMat);
                        clamp.rotation.z = Math.PI / 2;
                        clamp.position.set(hx, 1.25, 0.15);
                        engineGroup.add(clamp);
                    });

                    // 下水管
                    const lowerHose = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.26, 12), hoseMat);
                    lowerHose.rotation.z = Math.PI / 2;
                    lowerHose.position.set(-1.42, 0.52, -0.15);
                    engineGroup.add(lowerHose);
                    [-1.52, -1.32].forEach(hx => {
                        const clamp = new THREE.Mesh(new THREE.CylinderGeometry(0.044, 0.044, 0.02, 10), clampMat);
                        clamp.rotation.z = Math.PI / 2;
                        clamp.position.set(hx, 0.52, -0.15);
                        engineGroup.add(clamp);
                    });

                    // 9. 工業機油與柴油雙級濾清器 (Double Oil & Fuel Filters)
                    const filterMat = new THREE.MeshStandardMaterial({ color: 0x2563eb, metalness: 0.4, roughness: 0.3 }); // 標誌性藍色濾清器
                    [-0.95, -0.78].forEach(x => {
                        const filter = new THREE.Mesh(new THREE.CylinderGeometry(0.065, 0.065, 0.22, 12), filterMat);
                        filter.position.set(x, 0.72, 0.42);
                        engineGroup.add(filter);
                        const filterCap = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.07, 0.03, 12), frameMat);
                        filterCap.position.set(x, 0.82, 0.42);
                        engineGroup.add(filterCap);
                    });

                    // ── [NEW] 新增雙重啟動電瓶與紅黑高壓接線 ──────────────────────
                    const batteryMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, metalness: 0.6, roughness: 0.5 }); // 黑色電瓶殼
                    const copperWireMat = new THREE.MeshStandardMaterial({ color: 0xef4444, metalness: 0.8, roughness: 0.2 }); // 紅色高壓正極電纜
                    const groundWireMat = new THREE.MeshStandardMaterial({ color: 0x05070c, roughness: 0.8 }); // 黑色負極地線
                    
                    [-0.24, 0.24].forEach(zOffset => {
                        // 啟動蓄電池組 (2組，置於發電機底座旁)
                        const batBox = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.18, 0.18), batteryMat);
                        batBox.position.set(0.65, 0.45, zOffset);
                        engineGroup.add(batBox);

                        // 電瓶金屬端子螺帽與正負極彩色保護蓋
                        [-0.07, 0.07].forEach(bx => {
                            const termMat = bx < 0 ? copperWireMat : groundWireMat;
                            const batTerm = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.02, 8), termMat);
                            batTerm.position.set(0.65 + bx, 0.54, zOffset);
                            engineGroup.add(batTerm);
                            
                            // 螺帽固定小墊片
                            const washer = new THREE.Mesh(new THREE.CylinderGeometry(0.026, 0.026, 0.005, 6), APP.materials.aluminum);
                            washer.position.set(0.65 + bx, 0.53, zOffset);
                            engineGroup.add(washer);
                        });

                        // 橫跨電池上方的金屬固定壓板與拉桿螺栓 (Battery Clamps)
                        const strap = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.012, 0.22), APP.materials.aluminum);
                        strap.position.set(0.65, 0.542, zOffset);
                        engineGroup.add(strap);
                        [-0.1, 0.1].forEach(bz => {
                            const rod = new THREE.Mesh(new THREE.CylinderGeometry(0.006, 0.006, 0.2, 6), APP.materials.aluminum);
                            rod.position.set(0.65, 0.44, zOffset + bz);
                            engineGroup.add(rod);
                        });

                        // 高壓電纜線管 (紅/黑粗線模擬啟動線路，連接至引擎起步機，加粗並美化)
                        const cable = new THREE.Mesh(new THREE.BoxGeometry(0.024, 0.35, 0.024), zOffset < 0 ? groundWireMat : copperWireMat);
                        cable.position.set(0.62, 0.68, zOffset);
                        cable.rotation.z = -0.32;
                        engineGroup.add(cable);
                    });

                    // ── [NEW] 引擎進氣系統與雙側高精度空氣濾清器筒 (Dual Air Filters & Intake Manifold) ────
                    const filterBodyMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, roughness: 0.8 }); // 黑色消光濾筒
                    const tubeMat = new THREE.MeshStandardMaterial({ color: 0xcbd5e1, metalness: 0.9, roughness: 0.2 }); // 亮銀色進氣岐管
                    
                    [-0.32, 0.32].forEach(zOffset => {
                        // 進氣濾清器雙圓筒 (Air Intake Filters)
                        const airFilter = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.13, 0.32, 16), filterBodyMat);
                        airFilter.rotation.x = Math.PI / 2;
                        airFilter.position.set(-1.12, 1.36, zOffset);
                        engineGroup.add(airFilter);

                        // 濾清器亮銀束環固定帶 (Clamps)
                        const clamp = new THREE.Mesh(new THREE.CylinderGeometry(0.136, 0.136, 0.02, 16), tubeMat);
                        clamp.rotation.x = Math.PI / 2;
                        clamp.position.set(-1.12, 1.36, zOffset);
                        engineGroup.add(clamp);

                        // 連接至渦輪的 90度 亮銀彎管 (Bent Intake Tubes)
                        const runTube = new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.065, 0.065), tubeMat);
                        runTube.position.set(-0.88, 1.48, zOffset * 0.6);
                        runTube.rotation.z = 0.25;
                        engineGroup.add(runTube);
                    });

                    // ── [NEW] 高精度發電機頂部大功率接線盒 (Terminal Wiring Box) ─────────────────────
                    const junctionBox = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.34, 0.55), alternatorMat);
                    junctionBox.position.set(1.15, 1.46, 0); // 置於發電機組頂部
                    engineGroup.add(junctionBox);

                    // 接線盒側邊螺栓
                    for(let xB=0; xB<2; xB++) {
                        [-0.22, 0.22].forEach(zb => {
                            const bolt = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.02, 6), APP.materials.aluminum);
                            bolt.rotation.x = Math.PI / 2;
                            bolt.position.set(1.0 + xB * 0.3, 1.46, zb);
                            engineGroup.add(bolt);
                        });
                    }

                    // ── [NEW] 引擎控制壓力/機油溫度實體指針儀表板 (Mechanical Dial Gauges) ─────────────
                    const dialFrame = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.22, 0.02), frameMat);
                    dialFrame.position.set(-0.35, 1.25, 0.44); // 面向通道側的外側表面
                    engineGroup.add(dialFrame);

                    // 三個彩色迷你圓指針儀表 (Dials)
                    [0xef4444, 0x3b82f6, 0x10b981].forEach((color, idx) => {
                        const dialPlate = new THREE.Mesh(new THREE.CylinderGeometry(0.042, 0.042, 0.008, 12), new THREE.MeshBasicMaterial({ color: 0xffffff }));
                        dialPlate.rotation.x = Math.PI / 2;
                        dialPlate.position.set(-0.46 + idx * 0.11, 1.25, 0.452);
                        engineGroup.add(dialPlate);

                        // 紅色指針 (Needles)
                        const needle = new THREE.Mesh(new THREE.BoxGeometry(0.004, 0.038, 0.004), new THREE.MeshBasicMaterial({ color: 0xdc2626 }));
                        needle.rotation.z = -0.5 + idx * 0.6;
                        needle.position.set(-0.46 + idx * 0.11, 1.25, 0.458);
                        engineGroup.add(needle);
                    });

                    // ── [NEW DETAIL] 高壓共軌柴油噴射系統管道 (Common Rail Fuel Injection Lines) ──
                    const railTube = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 1.5, 8), APP.materials.aluminum);
                    railTube.rotation.z = Math.PI / 2;
                    railTube.position.set(-0.35, 1.34, 0.28);
                    engineGroup.add(railTube);
                    
                    for (let p = 0; p < 4; p++) {
                        const fuelPipe = new THREE.Mesh(new THREE.CylinderGeometry(0.005, 0.005, 0.12, 6), APP.materials.copper);
                        fuelPipe.position.set(-0.85 + p * 0.34, 1.38, 0.24);
                        fuelPipe.rotation.x = -0.3;
                        engineGroup.add(fuelPipe);
                    }

                    // ── [NEW DETAIL] 引擎-發電機聯軸器飛輪防護外殼與轉軸 (Engine-Alternator Coupling & Flywheel Housing) ──
                    const couplingHousing = new THREE.Mesh(new THREE.CylinderGeometry(0.38, 0.38, 0.18, 24), frameMat);
                    couplingHousing.rotation.z = Math.PI / 2;
                    couplingHousing.position.set(0.52, 0.88, 0);
                    engineGroup.add(couplingHousing);

                    const innerShaft = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.24, 12), APP.materials.aluminum);
                    innerShaft.rotation.z = Math.PI / 2;
                    innerShaft.position.set(0.52, 0.88, 0);
                    engineGroup.add(innerShaft);

                    // ── [NEW DETAIL] 黃色手拉式機油量尺針 (Engine Oil Dipstick) ──
                    const dipstickLoop = new THREE.Mesh(new THREE.TorusGeometry(0.022, 0.006, 6, 12), canopyMat);
                    dipstickLoop.position.set(-0.08, 1.05, 0.44);
                    dipstickLoop.rotation.y = Math.PI / 2;
                    engineGroup.add(dipstickLoop);

                    const dipstickTube = new THREE.Mesh(new THREE.CylinderGeometry(0.008, 0.008, 0.26, 8), APP.materials.aluminum);
                    dipstickTube.rotation.z = -0.3;
                    dipstickTube.position.set(-0.04, 0.94, 0.44);
                    engineGroup.add(dipstickTube);

                    // ── [NEW DETAIL] 電子控制單元 (ECU) 與彩色感測線束 (Engine Control Unit & Wiring Harness) ──
                    const ecuBox = new THREE.Mesh(new THREE.BoxGeometry(0.24, 0.2, 0.08), APP.materials.aluminum);
                    ecuBox.position.set(0.1, 0.98, -0.44);
                    engineGroup.add(ecuBox);

                    const wireColors = [0xef4444, 0x3b82f6, 0x22c55e, 0xfbbf24];
                    wireColors.forEach((colorCode, wIdx) => {
                        const wireMat = new THREE.MeshStandardMaterial({ color: colorCode, roughness: 0.5 });
                        const wire = new THREE.Mesh(new THREE.BoxGeometry(0.008, 0.12, 0.008), wireMat);
                        wire.position.set(0.04 + wIdx * 0.03, 0.84, -0.44);
                        wire.rotation.z = 0.2 - wIdx * 0.1;
                        engineGroup.add(wire);
                    });

                    genset.add(engineGroup);

                    // ── 建立外殼防護群組 (Canopy Assembly) ──────────────────────────
                    const canopyAssembly = new THREE.Group();

                    // 3. 主機防音外殼 (Canopy)
                    const canopyGeo = new THREE.BoxGeometry(4.8, 1.8, 1.8);
                    const canopy = new THREE.Mesh(canopyGeo, canopyMat);
                    canopy.position.set(0, 1.3, 0);
                    canopyAssembly.add(canopy);
                    ...
                    // 兩側共部署 6 扇檢修門
                    canopyAssembly.add(createDoor(-1.4, 0.9, 0));
                    canopyAssembly.add(createDoor(0, 0.9, 0));
                    canopyAssembly.add(createDoor(1.4, 0.9, 0));
                    canopyAssembly.add(createDoor(-1.4, -0.9, Math.PI));
                    canopyAssembly.add(createDoor(0, -0.9, Math.PI));
                    canopyAssembly.add(createDoor(1.4, -0.9, Math.PI));

                    // 5. 前端高密度散熱水箱百葉 (Radiator Louvers) - 空心外框與高對比金屬葉片
                    const grilleGroup = new THREE.Group();
                    // 深色凹槽背景板，強化鏤空陰影感
                    const grilleBack = new THREE.Mesh(new THREE.BoxGeometry(0.06, 1.42, 1.52), grilleBackMat);
                    grilleBack.position.set(0.02, 0, 0);
                    grilleGroup.add(grilleBack);
                    
                    // 空心外框四件套 (組合成與原 0.1 x 1.5 x 1.6 完全相同的外形界限)
                    const borderTop = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.06, 1.6), frameMat);
                    borderTop.position.set(0, 0.72, 0);
                    const borderBottom = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.06, 1.6), frameMat);
                    borderBottom.position.set(0, -0.72, 0);
                    const borderLeft = new THREE.Mesh(new THREE.BoxGeometry(0.08, 1.5, 0.06), frameMat);
                    borderLeft.position.set(0, 0, -0.77);
                    const borderRight = new THREE.Mesh(new THREE.BoxGeometry(0.08, 1.5, 0.06), frameMat);
                    borderRight.position.set(0, 0, 0.77);
                    grilleGroup.add(borderTop, borderBottom, borderLeft, borderRight);
                    
                    // 亮銀灰色金屬葉片 (Louver Blades)
                    for(let i=0; i<12; i++) {
                        const louver = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.02, 1.5), louverBladeMat);
                    ...
                    canopyAssembly.add(grilleGroup);

                    // 6. 側邊數位控制面板與緊急停止按鈕 (Side HMI Control Panel & E-Stop)
                    const hmiGroup = new THREE.Group();
                    // 控制箱體 (極薄壁掛式，貼合在黃色外殼側邊)
                    const cBox = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.8, 0.12), darkPanelMat);
                    hmiGroup.add(cBox);
                    // HMI 邊框與 LCD 螢幕
                    const bezel = new THREE.Mesh(new THREE.BoxGeometry(0.48, 0.34, 0.02), rubberMat);
                    bezel.position.set(-0.06, 0.16, 0.06);
                    hmiGroup.add(bezel);
                    const lcd = new THREE.Mesh(new THREE.PlaneGeometry(0.44, 0.3), screenMat);
                    lcd.position.set(-0.06, 0.16, 0.071);
                    hmiGroup.add(lcd);
                    // E-Stop 紅色按鈕與黃色底座
                    const eStopBase = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.02, 10), new THREE.MeshStandardMaterial({color: 0xeab308}));
                    eStopBase.rotation.x = Math.PI / 2;
                    eStopBase.position.set(0.22, -0.16, 0.06);
                    const eStopBtn = new THREE.Mesh(new THREE.CylinderGeometry(0.028, 0.028, 0.04, 10), dangerMat);
                    eStopBtn.rotation.x = Math.PI / 2;
                    eStopBtn.position.set(0.22, -0.16, 0.08);
                    hmiGroup.add(eStopBase, eStopBtn);
                    // 狀態指示燈
                    for (let i = 0; i < 2; i++) {
                        const light = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.02, 8), new THREE.MeshStandardMaterial({color: i===0?0x10b981:0xef4444, emissive: i===0?0x10b981:0xef4444, emissiveIntensity: 0.5}));
                        light.rotation.x = Math.PI / 2;
                        light.position.set(0.22, 0.04 + (i * 0.12), 0.06);
                        hmiGroup.add(light);
                    }
                    // 將整個控制箱體放置於黃色外殼的右側（z = 0.9m），靠近發電機端（x = 1.6m）
                    hmiGroup.position.set(1.6, 1.25, 0.9);
                    canopyAssembly.add(hmiGroup);

                    // 10. [NEW] 後端發電機室冷風吸氣口百葉 (Air Intake Louvers) - 空心外框與高對比金屬葉片
                    const intakeGrilleGroup = new THREE.Group();
                    // 深色凹槽背景板，強化鏤空陰影感
                    const intakeBack = new THREE.Mesh(new THREE.BoxGeometry(0.06, 1.42, 1.52), grilleBackMat);
                    intakeBack.position.set(-0.02, 0, 0);
                    intakeGrilleGroup.add(intakeBack);
                    
                    // 空心外框四件套 (組合成與原 0.1 x 1.5 x 1.6 完全相同的外形界限)
                    const intakeBorderTop = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.06, 1.6), frameMat);
                    intakeBorderTop.position.set(0, 0.72, 0);
                    const intakeBorderBottom = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.06, 1.6), frameMat);
                    intakeBorderBottom.position.set(0, -0.72, 0);
                    const intakeBorderLeft = new THREE.Mesh(new THREE.BoxGeometry(0.08, 1.5, 0.06), frameMat);
                    intakeBorderLeft.position.set(0, 0, -0.77);
                    const intakeBorderRight = new THREE.Mesh(new THREE.BoxGeometry(0.08, 1.5, 0.06), frameMat);
                    intakeBorderRight.position.set(0, 0, 0.77);
                    intakeGrilleGroup.add(intakeBorderTop, intakeBorderBottom, intakeBorderLeft, intakeBorderRight);
                    
                    // 亮銀灰色金屬葉片 (Louver Blades)
                    for (let i = 0; i < 12; i++) {
                        const louver = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.02, 1.5), louverBladeMat);
                        louver.position.set(0.02, 0.65 - (i * 0.12), 0);
                        louver.rotation.z = -Math.PI / 6; // 傾斜百葉 (朝向下方，防止雨水吸入)
                        intakeGrilleGroup.add(louver);
                    }
                    intakeGrilleGroup.position.set(2.4, 1.3, 0);
                    canopyAssembly.add(intakeGrilleGroup);
                    ...
                    // 避震雙重金屬箍環 (Exhaust Muffler Clamps)
                    [-0.6, 0.6].forEach(xOff => {
                        const clampMat = APP.materials.aluminum;
                        const ring = new THREE.Mesh(new THREE.TorusGeometry(0.36, 0.016, 6, 32), clampMat);
                        ring.position.set(xOff, 0, 0);
                        ring.rotation.y = Math.PI / 2;
                        mufflerGroup.add(ring);
                        
                        // 箍環兩側的連接凸耳與固定螺栓 (Bolted Flange Ear)
                        [-0.37, 0.37].forEach(zOff => {
                            const ear = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.06, 0.04), frameMat);
                            ear.position.set(xOff, 0, zOff);
                            mufflerGroup.add(ear);
                            const bolt = new THREE.Mesh(new THREE.CylinderGeometry(0.01, 0.01, 0.08, 6), APP.materials.aluminum);
                            bolt.position.set(xOff, 0, zOff);
                            mufflerGroup.add(bolt);
                        });
                    });

                    // 固定支架 (Flanges/Brackets)
                    const bracketGeo = new THREE.BoxGeometry(0.1, 0.2, 0.65);
                    ...
                    // 為 bracket 加裝固定基座墊片與避震彈簧 (Vibration Isolator Springs)
                    [-0.6, 0.6].forEach(xOff => {
                        [-0.22, 0.22].forEach(zOff => {
                            // 避震底座
                            const isoBase = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.04, 0.08), frameMat);
                            isoBase.position.set(xOff, -0.42, zOff);
                            mufflerGroup.add(isoBase);
                            
                            // 避震彈簧 (以亮金屬圓柱體代替)
                            const spring = new THREE.Mesh(new THREE.CylinderGeometry(0.022, 0.022, 0.08, 8), APP.materials.aluminum);
                            spring.position.set(xOff, -0.38, zOff);
                            mufflerGroup.add(spring);
                        });
                    });
                    // 垂直排氣管 (連接引擎)
                    ...
                    canopyAssembly.add(mufflerGroup);
                    ...
                    canopyAssembly.add(dangerLabel);

                    genset.add(canopyAssembly);
                    genset.canopyAssembly = canopyAssembly;
                    genset.engineGroup = engineGroup;
                    genset.fanGroup = fanGroup;
                    genset.altFanGroup = altFanGroup;

                    // 9. [NEW] 排氣口冒煙效果粒子組 (Exhaust Smoke Particle Group)
                    const smokeGroup = new THREE.Group();
                    smokeGroup.position.set(-0.9, 3.4, 0);
                    const smokeMat = new THREE.MeshBasicMaterial({
                        color: 0x334155, // slate-600
                        transparent: true,
                        opacity: 0.0,
                        depthWrite: false
                    });
                    for (let i = 0; i < 15; i++) {
                        const particle = new THREE.Mesh(new THREE.SphereGeometry(0.05, 8, 8), smokeMat.clone());
                    ...
                    genset.add(smokeGroup);
                    genset.smokeGroup = smokeGroup;

                    // ── [NEW] 柴油日用油箱 (Diesel Day Tank) ──────────────────────────
                    const dayTank = new THREE.Group();
                    dayTank.position.set(0, 0, -1.55); // 位於發電機南側底座旁 (局部座標)

                    // 1. 油箱主體 (Tank Body) - 黃色金屬烤漆
                    const tankBodyMat = new THREE.MeshStandardMaterial({ color: 0xeab308, metalness: 0.5, roughness: 0.3 });
                    const tankBody = new THREE.Mesh(new THREE.BoxGeometry(0.72, 1.2, 0.72), tankBodyMat);
                    tankBody.position.y = 0.6;
                    tankBody.castShadow = true;
                    tankBody.receiveShadow = true;
                    dayTank.add(tankBody);

                    // 2. 油箱底座角架 (Tank Feet)
                    for (let tx = -0.32; tx <= 0.32; tx += 0.64) {
                        for (let tz = -0.32; tz <= 0.32; tz += 0.64) {
                            const foot = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.08, 6), frameMat);
                            foot.position.set(tx, 0.04, tz);
                            dayTank.add(foot);
                        }
                    }

                    // 3. 玻璃管油位計 (Fuel Level Gauge)
                    const gaugeFrameMat = new THREE.MeshStandardMaterial({ color: 0x222222, roughness: 0.8 });
                    const glassMat = new THREE.MeshStandardMaterial({ color: 0xffffff, transparent: true, opacity: 0.3, roughness: 0.1 });
                    const fuelRedMat = new THREE.MeshBasicMaterial({ color: 0xef4444 }); // 紅色油位線

                    // 儀表背板
                    const gaugeBack = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.9, 0.02), gaugeFrameMat);
                    gaugeBack.position.set(0.365, 0.6, 0);
                    gaugeBack.rotation.y = Math.PI / 2; // 面向外側 (Z = -1.55 面向南側)
                    dayTank.add(gaugeBack);

                    // 玻璃保護管
                    const glassTube = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.8, 8), glassMat);
                    glassTube.position.set(0.38, 0.6, 0);
                    dayTank.add(glassTube);

                    // 紅色液位線 (油位 85%)
                    const fuelLevel = new THREE.Mesh(new THREE.CylinderGeometry(0.008, 0.008, 0.68, 8), fuelRedMat);
                    fuelLevel.position.set(0.38, 0.6 - 0.06, 0);
                    dayTank.add(fuelLevel);

                    // 4. 頂部注油口與進油法蘭 (Filler & Inlets)
                    const inletPipe = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.12, 8), pipeMat);
                    inletPipe.position.set(-0.2, 1.25, -0.2);
                    dayTank.add(inletPipe);
                    const inletCap = new THREE.Mesh(new THREE.CylinderGeometry(0.045, 0.045, 0.02, 10), dangerMat); // 紅色防護蓋
                    inletCap.position.set(-0.2, 1.31, -0.2);
                    dayTank.add(inletCap);

                    // 5. 頂部呼吸閥/安全閥 (Breather Valve)
                    const breatherBody = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.16, 8), pipeMat);
                    breatherBody.position.set(0.2, 1.28, 0.2);
                    dayTank.add(breatherBody);
                    const breatherCap = new THREE.Mesh(new THREE.SphereGeometry(0.04, 12, 10, 0, Math.PI * 2, 0, Math.PI / 2), frameMat);
                    breatherCap.position.set(0.2, 1.36, 0.2);
                    dayTank.add(breatherCap);

                    // 6. 底部排污球閥 (Drain Valve)
                    const drainPipe = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.016, 0.08, 8), pipeMat);
                    drainPipe.position.set(0, -0.04, 0);
                    dayTank.add(drainPipe);

                    // 7. 聯絡柴油管線 (Copper Fuel Supply Pipeline)
                    const fuelMat = APP.materials.copper || new THREE.MeshStandardMaterial({ color: 0xb45309, metalness: 0.8, roughness: 0.2 });
                    
                    // 段1: 油箱到底部 (Z = 0.1 -> 0.7)
                    const fSeg1 = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.75, 8), fuelMat);
                    fSeg1.rotation.x = Math.PI / 2;
                    fSeg1.position.set(0.18, 0.06, 0.325);
                    dayTank.add(fSeg1);

                    // 段2: 橫行至發電機 (X = 0.18 -> -0.35)
                    const fSeg2 = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.53, 8), fuelMat);
                    fSeg2.rotation.z = Math.PI / 2;
                    fSeg2.position.set(-0.085, 0.06, 0.7);
                    dayTank.add(fSeg2);

                    // 段3: 立管直升進入發電機內部 (Y = 0.06 -> 1.32)
                    const fSeg3 = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 1.26, 8), fuelMat);
                    fSeg3.position.set(-0.35, 0.69, 0.7);
                    dayTank.add(fSeg3);

                    // 段4: 接管，從立管 (Z = 0.7) 連接到 common rail (Z = 0.28, Y = 1.32)
                    const fSeg4 = new THREE.Mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.42, 8), fuelMat);
                    fSeg4.rotation.x = Math.PI / 2;
                    fSeg4.position.set(-0.35, 1.32, 0.49);
                    dayTank.add(fSeg4);

                    // 將 Day Tank 加入 genset (直接作為 genset 的子物件)
                    genset.add(dayTank);
                ...
                this.generatorMesh = detailedGenerator;
            ...
            updateLiveValues() {
                ...
                // Update generator HMI dynamic display
                APP.updateGeneratorScreen();
            },
            ...
            onClick(e) {
                ...
                if (intersects.length > 0) {
                    ...
                    this.showDetail(hitObj.userData);

                    const parentGroup = hitObj.parent;
                    if (parentGroup) {
                        // 1. 動態計算特寫相機視點
                        const eqPos = new THREE.Vector3();
                        hitObj.getWorldPosition(eqPos);
                        
                        let targetPos, camPos;
                        if (hitObj.userData.type === 'Generator') {
                            // 工業級發電機特寫：切換至戶外無遮擋的黃金三角視角
                            targetPos = new THREE.Vector3(eqPos.x, 1.3, eqPos.z);
                            camPos = new THREE.Vector3(eqPos.x + 3.8, 2.8, eqPos.z + 3.5);
                        } else {
                            // 配電盤、UPS、電池櫃特寫
                            targetPos = new THREE.Vector3(eqPos.x, 1.2, eqPos.z);
                            camPos = new THREE.Vector3(eqPos.x, 1.8, eqPos.z + 2.5);
                        }
                        this.tweenCamera(camPos, targetPos);

                        // 2. 開門與拆解動畫處理
                        if (hitObj.userData.type === 'Generator') {
                            this.toggleGeneratorExplode(parentGroup);
                        } else {
                            // 關閉發電機的拆解狀態（如果點選了非發電機設備）
                            this.scene.traverse((obj) => {
                                if (obj.canopyAssembly && obj.isExploded) {
                                    if (obj.parent) {
                                        this.toggleGeneratorExplode(obj.parent);
                                    }
                                }
                            });

                            if (parentGroup.doorGroup) {
                                // 關閉其他打開的門
                                this.scene.traverse((obj) => {
                                    if (obj.doorGroup && obj !== parentGroup && obj.doorState === 'open') {
                                        this.toggleDoor(obj);
                                    }
                                });
                                
                                // 切換當前設備的門板狀態
                                this.toggleDoor(parentGroup);
                            }
                        }
                    }
                }
            ...
            toggleDoor(group) {
                if (!group.doorGroup) return;
                
                // 初始化狀態
                if (group.doorState === undefined) {
                    group.doorState = 'closed';
                }
                
                const targetRotation = group.doorState === 'closed' ? -Math.PI * 0.65 : 0;
                group.doorState = group.doorState === 'closed' ? 'open' : 'closed';
                
                if (window.TWEEN) {
                    new TWEEN.Tween(group.doorGroup.rotation)
                        .to({ y: targetRotation }, 1000)
                        .easing(TWEEN.Easing.Quadratic.Out)
                        .start();
                } else {
                    group.doorGroup.rotation.y = targetRotation;
                }
            },

            toggleGeneratorExplode(group) {
                // 在 group 中尋找 detailedGenerator 
                let detailedGenerator = null;
                group.children.forEach(child => {
                    if (child.canopyAssembly) {
                        detailedGenerator = child;
                    }
                });
                
                if (!detailedGenerator || !detailedGenerator.canopyAssembly) return;
                
                if (detailedGenerator.isExploded === undefined) {
                    detailedGenerator.isExploded = false;
                }
                
                const targetY = detailedGenerator.isExploded ? 0 : 3.2;
                detailedGenerator.isExploded = !detailedGenerator.isExploded;
                
                if (window.TWEEN) {
                    new TWEEN.Tween(detailedGenerator.canopyAssembly.position)
                        .to({ y: targetY }, 1200)
                        .easing(TWEEN.Easing.Cubic.Out)
                        .start();
                } else {
                    detailedGenerator.canopyAssembly.position.y = targetY;
                }
            },
            ...
            closeDetail() {
                ...
                // 自動關閉所有開啟的門
                this.scene.traverse((obj) => {
                    if (obj.doorGroup && obj.doorState === 'open') {
                        this.toggleDoor(obj);
                    }
                });
            ...
            updateGeneratorScreen() {
                const canvas = APP.textures.generatorScreen ? APP.textures.generatorScreen.image : null;
                if (!canvas) return;
                const ctx = canvas.getContext('2d');
                if (!ctx) return;

                // 獲取當前數據
                const isRunning = APP.currentScenario === 'generator';
                const isCranking = APP.currentScenario === 'utilityFail';

                const elRpm = document.getElementById('gen_val_rpm');
                const elPress = document.getElementById('gen_val_press');
                const elTemp = document.getElementById('gen_val_temp');
                const elBatt = document.getElementById('gen_val_batt');

                const genRpm = elRpm ? parseFloat(elRpm.innerText || '0') : 0;
                const genPress = elPress ? parseFloat(elPress.innerText || '0') : 0;
                const genTemp = elTemp ? parseFloat(elTemp.innerText || '0') : 0;
                const genBatt = elBatt ? parseFloat(elBatt.innerText || '0') : 0;

                // 累加 HMI 內部 tick，用於動畫
                APP.generatorHmiTick = (APP.generatorHmiTick || 0) + 1;
                ...
                if (isRunning) {
                    statusText = '● RUNNING / SUPPLYING';
                    statusColor = '#22c55e'; // 綠色
                } else if (isCranking) {
                    statusText = '● ENGINE CRANKING...';
                    statusColor = '#f59e0b'; // 橘色
                } else {
                    statusText = '● SYSTEM STANDBY - READY';
                    statusColor = '#38bdf8'; // 藍色
                }
                ...
                ctx.lineWidth = 1;
                ctx.beginPath();
                ctx.moveTo(10, 55); ctx.lineTo(502, 55);
                ctx.moveTo(10, 205); ctx.lineTo(502, 205);
                ctx.stroke();
                ...
                if (isRunning) {
                    // 電壓與頻率隨 RPM 微幅抖動
                    volt = (478 + Math.random() * 4).toFixed(1);
                    hz = (genRpm / 25).toFixed(2);
                } else if (isCranking) {
                    volt = (150 + Math.random() * 80).toFixed(0);
                    hz = (genRpm / 25).toFixed(1);
                }

                // 數據顯示
                ctx.fillText(`Engine Speed:  ${genRpm.toFixed(0)} RPM`, 20, 85);
                ctx.fillText(`Gen Voltage:   ${volt} V`, 20, 115);
                ctx.fillText(`Frequency:     ${hz} Hz`, 20, 145);
                ctx.fillText(`Oil Pressure:  ${genPress.toFixed(1)} Bar`, 20, 175);

                // 電瓶與引擎溫度
                ctx.font = '15px monospace';
                ctx.fillStyle = '#94a3b8';
                ctx.fillText(`Battery: ${genBatt.toFixed(1)} V   Engine Temp: ${genTemp.toFixed(1)} °C`, 20, 225);
                ...
                if (isRunning || isCranking) {
                    ...
                    ctx.lineWidth = 1;
                    ctx.beginPath();
                    // grid lines inside the waveform window (x: 280 to 490, y: 70 to 170)
                    for (let x = 280; x <= 490; x += 30) {
                        ctx.moveTo(x, 70); ctx.lineTo(x, 170);
                    }
                    for (let y = 70; y <= 170; y += 25) {
                        ctx.moveTo(280, y); ctx.lineTo(490, y);
                    }
                    ctx.stroke();

                    // Waveform line
                    ctx.strokeStyle = isRunning ? '#22c55e' : '#f59e0b';
                    ctx.lineWidth = 2;
                    ctx.beginPath();
                    const phase = APP.generatorHmiTick * 0.45;
                    const amp = isRunning ? 22 : 10;
                    const freq = isRunning ? 0.08 : 0.04;
                    for (let x = 280; x <= 490; x++) {
                        const y = 120 + Math.sin((x - 280) * freq + phase) * amp;
                    ...
                    ctx.fillText('LIVE AC WAVEFORM', 280, 63);
                } else {
                    // Draw a flat line for standby
                    ...
                    ctx.font = 'bold 11px monospace';
                    ctx.fillText('NO AC SIGNAL', 280, 63);
                }

                // Draw Day Tank Fuel level bar (x: 280 to 490, y: 185 to 200)
                ctx.fillStyle = '#111827';
                ctx.fillRect(280, 185, 210, 16);
                
                // Fuel bar color (standard diesel amber/orange color)
                ctx.fillStyle = '#b45309';
                ctx.fillRect(280, 185, Math.round(210 * 0.85), 16);

                ctx.strokeStyle = '#374151';
                ctx.lineWidth = 1;
                ctx.strokeRect(280, 185, 210, 16);

                ctx.fillStyle = '#f8fafc';
                ctx.font = 'bold 11px monospace';
                ctx.fillText('FUEL DAY TANK: 85%', 288, 197);

                // 底部閃爍狀態點
                ctx.fillStyle = statusColor;
                ctx.beginPath();
                ctx.arc(490, 222, 6, 0, Math.PI * 2);
                ctx.fill();

                APP.textures.generatorScreen.needsUpdate = true;
            },
            ...
            setScenario(key) {
                ...
                // Update generator detailed telemetry based on scenario
                const genRpmEl = document.getElementById('gen_val_rpm');
                const genPressEl = document.getElementById('gen_val_press');
                const genTempEl = document.getElementById('gen_val_temp');
                const genBattEl = document.getElementById('gen_val_batt');

                if (key === 'generator') {
                    if (genRpmEl) { genRpmEl.setAttribute('data-base', 1500); genRpmEl.setAttribute('data-var', 5); }
                    if (genPressEl) { genPressEl.setAttribute('data-base', 4.5); genPressEl.setAttribute('data-var', 0.15); }
                    if (genTempEl) { genTempEl.setAttribute('data-base', 82.5); genTempEl.setAttribute('data-var', 1.0); }
                    if (genBattEl) { genBattEl.setAttribute('data-base', 28.2); genBattEl.setAttribute('data-var', 0.15); }
                } else if (key === 'utilityFail') {
                    if (genRpmEl) { genRpmEl.setAttribute('data-base', 450); genRpmEl.setAttribute('data-var', 50); }
                    if (genPressEl) { genPressEl.setAttribute('data-base', 2.8); genPressEl.setAttribute('data-var', 0.3); }
                    if (genTempEl) { genTempEl.setAttribute('data-base', 43.0); genTempEl.setAttribute('data-var', 0.3); }
                    if (genBattEl) { genBattEl.setAttribute('data-base', 24.2); genBattEl.setAttribute('data-var', 0.4); }
                } else {
                    // normal or bypass (standby)
                    if (genRpmEl) { genRpmEl.setAttribute('data-base', 0); genRpmEl.setAttribute('data-var', 0); }
                    if (genPressEl) { genPressEl.setAttribute('data-base', 0); genPressEl.setAttribute('data-var', 0); }
                    if (genTempEl) { genTempEl.setAttribute('data-base', 42.5); genTempEl.setAttribute('data-var', 0.2); }
                    if (genBattEl) { genBattEl.setAttribute('data-base', 27.2); genBattEl.setAttribute('data-var', 0.1); }
                }
            ...
            setCamera(preset) {
                ...
                else if(preset === 'tray')   { cp = new THREE.Vector3(0, 3.8, 6.5); ct = new THREE.Vector3(0, 1.8, -3.0); }
                else if(preset === 'generator') { cp = new THREE.Vector3(16.3, 3.2, -1.5); ct = new THREE.Vector3(12.5, 1.0, -5.0); }
            ...
            tweenCamera(pos, target) {
                if(window.TWEEN) {
                    // 鎖定 OrbitControls 避免內置緩動算法與 TWEEN 衝突導致畫面彈開/旋轉
            ...
            animate() {
                ...
                // Generator startup effects & animations!
                if (this.generatorMesh) {
                    const isRunning = this.currentScenario === 'generator';
                    const isCranking = this.currentScenario === 'utilityFail';
                    if (isRunning || isCranking) {
                        const speed = isRunning ? 0.35 : 0.12;
                        
                        // 1. Fan Spin
                        if (this.generatorMesh.fanGroup) this.generatorMesh.fanGroup.rotation.x += speed;
                        if (this.generatorMesh.altFanGroup) this.generatorMesh.altFanGroup.rotation.x += speed;
                        
                        // 2. Engine vibration (vibrate engine block independently inside canopy!)
                        if (this.generatorMesh.engineGroup) {
                            const amp = isRunning ? 0.005 : 0.012;
                            const time = Date.now() * 0.08;
                            this.generatorMesh.engineGroup.position.y = Math.sin(time) * amp;
                            this.generatorMesh.engineGroup.position.x = Math.cos(time * 0.8) * amp * 0.5;
                        }

                        // 3. Exhaust smoke particle plume (rising, expanding, and fading slate grey smoke)
                        if (this.generatorMesh.smokeGroup) {
                            this.generatorMesh.smokeGroup.visible = true;
                            this.generatorMesh.smokeGroup.children.forEach(particle => {
                                ...
                                particle.scale.addScalar(particle.userData.growSpeed);
                                if (particle.material) {
                                    particle.material.opacity = Math.max(0, 0.6 - (particle.position.y * 0.4));
                        ...
                        // Reset generator to standby state
                        if (this.generatorMesh.engineGroup) this.generatorMesh.engineGroup.position.set(0, 0, 0);
                        if (this.generatorMesh.smokeGroup) this.generatorMesh.smokeGroup.visible = false;
                    }
                }
